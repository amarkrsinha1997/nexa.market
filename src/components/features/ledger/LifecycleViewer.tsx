...
interface LifecycleViewerProps {
    ...
    orderUserId?: string; // To identify if actor is the user
}

export default function LifecycleViewer({ lifecycle, orderUserId }: LifecycleViewerProps) {
        ...
        <div className="space-y-2 bg-[#0f1016] rounded-xl border border-gray-800/50">
            <div className="space-y-0divide-y divide-gray-800">
                {lifecycle.map((event, index) => {
                    // Define User actions strictly
                    const isUserAction = ['ORDER_CREATED', 'PAYMENT_CONFIRMED'].includes(event.action);

                    // Fallback: If action is ambiguous, check ID/Name, but mostly rely on action type
                    const isUser = isUserAction || (event.actorId === orderUserId && !['CHECK', 'APPROVE', 'REJECT', 'RELEASE_PAYMENT'].includes(event.action));
                        ...
                        >
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                        ...
                                        <span className="text-sm font-medium text-white">
                                            {isUser ? "User" : (
                                                <span className="flex items-center gap-1">
                                                    ...
                                                    <span className="text-xs text-blue-400 font-normal bg-blue-500/10 px-1.5 py-0.5 rounded ml-1">
                                                        Admin
                                                    </span>
                                                </span>
                                            )}
                                    ...
                                    {/* Hide Email/Details for User events */}
                                    {!isUser && event.actorEmail && (
